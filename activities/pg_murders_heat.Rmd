...
```{r}
kill_and_heat <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_murder_temps.csv")
```
...
```{r}
kill_and_heat |>
  summarise(correlation = cor(murders, temperature, method="pearson"))
```
...
kill_and_heat |> 
  ...
  geom_point(aes(x=temperature, y=murders, label=month)) +
  geom_smooth(aes(x=temperature, y=murders), method="lm") +
  geom_text(aes(x=temperature, y=murders, label=month), size=4, vjust=-0.5)
```

### Task 6: Describe the results of both the correlation calculation and the scatterplot.

---
The correlation result of 0.58 and the sloped trend line on the scatter plot indicate that temperature and murders somewhat have a correlation, but temperature alone may not be a dominant factors that drive the number of murders up.  
---
