...
```{r}
gdp_djia |>
  summarise(correlation = cor(GDP, DJIA, method="pearson"))
```
...
gdp_djia |>
  ...
  labs(title = "GDP shows a strong correlation with Dow Jones Industrial Average",
...
---
The correlation result of 0.953 and the sloped trend line on the scatterplot that is nearly 45 degrees to the X-axis indicate that these two variables, GDP and DJIA, have a strong correlation. Nonetheless, not all the data points align perfectly on the trend line, so we cannot definitely say that one variable is a dominant factor that influences the other one.    
---
